...
public:
  ...
  VALUE get(KEY key) {
	...
	assert(nullptr != node); // key must be in the tree.
    // just use the node's value getter.
    return node->value();
  }
  ...
  void erase(KEY key) {
    // Use recursive helper functions to do all the work.
	assert(search(key, _root) != nullptr); // if key isn't in the tree we can't erase it.
...
private:
  ...
  BSTNode<KEY, VALUE>* remove(KEY query, BSTNode<KEY, VALUE>* subtree) {
    ...
    if (query < key) {
      ...
      subtree->set_right(remove(successor->key(), right));
      return subtree;
    }
  }
